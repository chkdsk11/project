    ...
    /*private*/ function sendError($error, stdClass $context) {
        ...
        try {
            if ($this->onSendError !== null) {
                ...
                if ($e instanceof Exception || $e instanceof Throwable || $e instanceof  \Error) {
                    $error = $e;
                }
            }
        }
        ...
        $writer->writeString($this->debug ? json_encode(['status'=>HttpStatus::SYS_ERROR,'explain'=>HttpStatus::$HttpStatusMsg[HttpStatus::SYS_ERROR],'data' => $res_data],JSON_UNESCAPED_UNICODE) : json_encode(['status'=>HttpStatus::SYS_ERROR,'explain'=>HttpStatus::$HttpStatusMsg[HttpStatus::SYS_ERROR],'data' => []],JSON_UNESCAPED_UNICODE));

        $soa_url = Di::getDefault()->get('SOAURL');
        $client = new Client($soa_url.'/service/log',false);
        $client->save([
           'prefix'=>'sys',
            'data'=>$res_data
        ]);
            ...
            if ($this->onBeforeInvoke !== null) {
                ...
                if (Future\isFuture($value)) {
                    return $value->then(function($value) use ($self, $name, $args, $context) {
                        if ($value instanceof Exception || $value instanceof Throwable || $value instanceof \Error) {
                            throw $value;
                        }
