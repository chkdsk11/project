            ...
            <div class="col-xs-12">
                        ...
                        <form action="" method="get">
                            <div class="tools-box" >
                                ...
                                <label class="clearfix sku_label">
                                    <a  href="{{ export }}" class="btn btn-primary sku_label_search">导出</a>
                                </label>
                            </div>
                        </form>
                ...
                <div>
                    <table id="dynamic-table" class="table table-striped table-bordered table-hover">
                        <thead>
                        <tr>
                            <th class="center sku_menu_row5">新增用户</th>
                            <th class="center sku_menu_row5">已支付订单数</th>
                            <th class="center sku_menu_row5">销售额（即已支付订单金额）</th>
                            <th class="center sku_menu_row4">成交人数</th>
                            <th class="center sku_menu_row5">客单价</th>
                            <th class="center sku_menu_row5">复购率（不去重）</th>
                            <th class="center sku_menu_row4"> 复购率（去重）</th>
                        </tr>
                        </thead>
                        <tbody id="productRuleList">
                        {% if info is defined %}
                        <tr>
                            <td class="text-center">{{ info['add_user'] }}</td>
                            <td class="text-center">{{ info['pay_order'] }}</td>
                            <td class="text-center">{{ info['price_count'] }}</td>
                            <td class="text-center">{{ info['bargain_num'] }}</td>
                            <td class="text-center">{{ info['order_avg'] }} </td>
                            <td class="text-center">{{ info['repeat_or'] }}</td>
                            <td class="text-center">{{ info['repeat_us'] }}</td>
                        </tr>
                        {% else %}
                        <tr>
...
<script src="http://{{ config.domain.static }}/assets/admin/js/apporder/pubilc.js"></script>
        {% endblock %}