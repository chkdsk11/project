...
<li class="active"><a href="/frontcategory/pclist">分类管理</a></li>
<li class="active">pc分类列表</li>
{% endblock %}
...
<table id="myTable" class="table table-bordered table-hover dataTable no-footer" role="grid">
    ...
    <tr role="row">
        ...
        <th tabindex="0" style="width: 15%;">绑定内容</th>
        <th tabindex="0" style="width: 5%;">排序</th>
        <th tabindex="0" style="width: 5%;">商品数量</th>
        <th tabindex="0" style="width: 40%;position: relative;">操作
            <a href="/frontcate/pcaddfirst?type=pc" class="btn btn-success table-btn-sm" id="addFrist">
...
<script>
    $(function(){
        $.getJSON('/frontcate/getcate?type=pc', function(data){
            var arr = [];
        ...
        function createTable(data) {
            ...
            for(var i = 0, len = data.length; i < len; i++){
                ...
                html += '<i class="'+ cls +'" data-pid='+ obj.pid +' data-id='+ obj.id +' data-level='+ obj.level +' data-son='+ obj.son +'></i>'+
                        ...
                        '<td>'+ obj.nums +'</td>'+
                        '<td>'+
                        '<a href="/frontcate/pcaddchild?type=pc&id='+obj.id+'" class="btn btn-success table-btn-sm">'+
                        '<i class="ace-icon glyphicon glyphicon-plus"></i>'+
                        '添加子分类'+
                        '</a>'+
                        '<a href="/frontcate/pcedit?type=pc&id='+obj.id+'" class="btn btn-success table-btn-sm">'+
...
//        }
        ...
        $(document).on('click','.is_enable_click',function(){
            ...
            $.getJSON('/frontcate/isSwitch?id='+id+'&is_enable='+is_enable+'&type=pc', function(msg){
