...
<li class="active"><a href="/grouporder/orderList">拼团订单列表</a></li>
<li class="active">
	订单
	...
	<div class="row bg-gray">
				...
				<div class="col-xs-12 padding-l-0">
					<div class="col-xs-6 padding-l-0">
						<p>
							订单状态：
							<span>
								{% if orderStat[orderStatus] is defined %}
									{{ orderStat[orderStatus] }}
								{% else %}
									未知状态
								{% endif %}
							</span>
							{% if alterState is defined and alterState == 1 and changeStat[orderStatus] is defined %}
								<a href="javascript:;" id="changeStatus" style="margin-right:80px;">修改状态</a>
							{% endif %}
							<select data-status="{% if changeStat[orderStatus] is defined %}{{ orderStatus }}{% endif %}" id="orderStatus">
								{% for k,v in changeStat %}
						...
						<p>用户手机：{{ orderInfo['phone'] }}</p>
						<p>
							用户身份：
							{% if ishead is defined and ishead == 1 %}
							开团人
							{% else %}
							参团人
							{% endif %}
						...
						{% if orderInfo['serviceSn'][0] is defined %}
						<p>
							关联服务单：
							{% for val in orderInfo['serviceSn'] %}
								<span>
						...
						<ul>
							{% for val in orderInfo['logistics'] %}
								<li {% if val['isEnd'] == 1 %}class="active"{% endif %}>
									<span class="date">
										{% if val['isFirst'] == 1 %}{{ val['date'] }}{% endif %}
									</span>
									<span class="day">
										{% if val['isFirst'] == 1 %}{{ val['week'] }}{% endif %}
									</span>
									<span class="remark">
		...
		<div class="col-xs-12 bg-white margin-b-20 order-detail-play">
			<div class="col-xs-4">
				...
				<p>
					实付金额：￥
					{% if orderInfo['total'] is defined and orderInfo['balance_price'] is defined %}
						{{ orderInfo['total'] - orderInfo['balance_price'] }}
					{% else %}
						0.00
					{% endif %}
				</p>
		...
		<div class="col-xs-12 order-detail-total-b">
			<div class="order-detail-total">
				<dl>
					<dt><span><?php echo $goodsCount; ?></span>件商品，总计：</dt>
					<dd>
						￥
						{% if orderInfo['total'] is defined and orderInfo['carriage'] is defined %}
							{{ orderInfo['total'] - orderInfo['carriage'] }}
						{% else %}
							0.00
						{% endif %}
					</dd>
