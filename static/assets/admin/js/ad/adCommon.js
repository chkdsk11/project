...
$(function() {
    ...
    $('#addAd').on('click',function(){
        ...
        if(endTime < nowTime){
            alert(endTime);alert(nowTime);
            layer_required('结束时间不能小于当前时间！');return false;
        }

        ad_position.each(function(){
            var adp_id = parseInt($(this).val());
            if (!isNaN(adp_id) && adp_id >0) {
                adp_ids.push(adp_id);
            }
        });
        if (adp_ids.length < 1) {
            ...
            $.post('/ad/ajax_check_is_group',{ad_position:adp_ids},function (msg) {
                if (msg.code == 300) {
                    layer_required('不能选择广告组，请选择广告位！');
                    ajax_status = false;
                }
                if (msg.code == 200) {
                    if (parseInt(advertisement_type) != msg.advertisement_type) {
                        layer_required('该广告位不允许添加'+$('#advertisement_type option:selected').html());
                        ajax_status = false;
                    }
                }
            });
...
$('#add_product').bind('click', function() {
    ...
    $('#product_list').append("<tr><td class='pid'>"+product_id+"</td><input type='hidden' name='product_id[]' value='"+product_id+"'/><td><input type='hidden' name='product_name[]' value='"+product_name+"'/>"+product_name+"</td><td><input type='file' name='product_image[]' value='上传' class='upload_image'></td><td><input type='text' name='order_product[]' class='product_order' size='5'></td><td><input type='button' value='删除' onclick='deleteok(this)' class='btn'/></td></tr>");
});
