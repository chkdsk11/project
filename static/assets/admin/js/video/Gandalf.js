    ...
    window.Producer = function () {
        ...
        this.tryProduce = function(initUrl, file, option, simpleAjax) {
            ...
            simpleAjax.get(initUrl, argument, function (reqData) {
                var isSub = reqData.indexOf('<');
                reqData = isSub != -1 ? reqData.substring(0, isSub) : reqData;
                try {
                ...
                if (data.code == 0) {
                    //全局
                    window.video_id = data.data.video_id;
                    window.video_unique = data.data.video_unique;
                    window.video_name = file.name;
