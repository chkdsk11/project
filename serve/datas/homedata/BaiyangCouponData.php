...
/**
 * Class BaiyangCouponData
 * @package Shop\Home\Datas
 * @param
 *      - platform 【pc、app、wap】 平台标识
 */
class BaiyangCouponData extends BaseData
{
    ...
    public function getCouponList($param,$goods_id = 0)
    {
        ...
        if(isset($param['user_id']) && !empty($param['user_id']) && ( !isset($param['is_temp']) || empty($param['is_temp']) || $param['is_temp'] == 0 )){
            $channelId = BaiyangCpsData::getInstance()->getUserChannelId($param['user_id']);
            $where.=$channelId ? "AND channel_id IN (0,{$channelId}) " : 'AND channel_id = 0 ';
            if($goods_id != 0){
                /*$tagInfo = $this->getData([
                    ...
                    'where' => 'where a.goods_id = :goods_id: AND c.status = 1 AND b.user_id = :user_id: AND a.platform_'.$param['platform'].' = :platform:',
                    'join' => 'LEFT JOIN \Shop\Models\BaiyangUserGoodsPriceTag as b on b.tag_id = a.tag_id LEFT JOIN \Shop\Models\BaiyangGoodsPriceTag as c on b.tag_id = c.tag_id'
                ],1);*/
//                if(isset($tagInfo['tag_id']) && !empty($tagInfo['tag_id']) && isset($tagInfo['goods_id']) && !empty($tagInfo['goods_id']) &&isset($tagInfo['platform_'.$param['platform']]) && !empty($tagInfo['platform_'.$param['platform']]) && $tagInfo['platform_'.$param['platform']] == 1){
                $tagInfo = $this->getData([
                    'table' => '\Shop\Models\BaiyangGoodsPriceTag as a',
                    'column' => 'b.tag_id',
                    'bind' => [
                        'user_id' => $param['user_id'],
                    ],
                    'where' => 'where a.status = 1 AND b.user_id = :user_id: AND b.status = 1',
                    'join' => 'LEFT JOIN \Shop\Models\BaiyangUserGoodsPriceTag as b on b.tag_id = a.tag_id '
                ]);
                $goods_tag_id = $tagInfo ? implode(',', array_column($tagInfo, 'tag_id')) : '';
                if ($goods_tag_id) {
                    $where.="AND ( goods_tag_id = 0 OR goods_tag_id in ({$goods_tag_id}))";
                }else{
            ...
            switch ($param['is_new_user']) {
                case '1':
                    $where.='AND ( ( group_set = :all_member: OR group_set = :new_member: ) AND register_bonus = :register_bonus:  )';
                    $data["all_member"] = 0;
                    $data["new_member"] = 1;
                    $data['register_bonus'] = 0;
                    break;
                case '0':
                    $where.='AND ( ( group_set = :all_member: OR group_set = :old_member: ) AND register_bonus = :register_bonus:  )';
                    $data["all_member"] =0;
                    $data["old_member"] =2;
                    $data['register_bonus'] = 0;
                    break;
            ...
            $data["new_member"] = 1;
            $data['register_bonus'] = 0;
            $where.='AND channel_id = 0 AND goods_tag_id = 0 ' ;
        }
        $result=$this->getData([
...
end_provide_time,coupon_value,min_cost,coupon_type,bring_number,coupon_number,relative_validity,limit_number',
     ...
     * @return array|bool
     */
    public function getUserCountCouponList($userId, $strCouponId)
    {
        $param = array(
            'table' => 'Shop\Models\BaiyangCouponRecord',
            'where' => "WHERE user_id = :user_id: AND coupon_sn = '{$strCouponId}'",
            'bind' => array('user_id'=>$userId)
        );
     ...
     * @author 邓永军
     */
    public function IsExistCouponInBrandData($brand_id,$platform)
    {
        $nowTime = time();
        ...
        foreach ($brand_arr as $new_brand_id){
            $param = [
                'table' => 'Shop\Models\BaiyangCoupon',
                'where' => 'WHERE ((FIND_IN_SET(:brand_ids:,brand_ids)) AND ( :nowTime: BETWEEN start_provide_time AND end_provide_time ) AND '.$platform.'_platform = 1 ) AND is_cancel = 0 AND channel_id = 0 AND goods_tag_id = 0 AND register_bonus = 0 AND group_set < 3',
                'bind' => [
    ...
    public function getCurrentUserCoupon($user_id,$platform)
    {
        ...
        if($isNewUser == 0){
            $group_set_str = ' AND (c.group_set = 0 OR c.group_set = 3 OR c.group_set = 2 OR c.register_bonus = 1 )';
        }else{
            $group_set_str = ' AND (c.group_set = 0 OR c.group_set = 3 OR c.group_set = 1)';
        }
        $UserAllCoupon = BaseData::getInstance()->getData([
            ...
            'where' => 'where c.is_cancel = :is_cancel: AND r.user_id = :user_id: AND r.is_used = :is_used: AND c.'.$platform.'_platform = :platform:'.$group_set_str,
            'bind' => [
                'is_cancel' => 0,
                'user_id' => $user_id,
                'is_used' => 0,
        ...
        if(!empty($UserAllCoupon)){
            foreach ($UserAllCoupon as $couponList){
                if ($couponList['validitytype'] == 1){
                    ...
                    $start_time = $couponList['start_use_time'];
                }else{
                    $over_time = $couponList['opposite_add_time'] + $couponList['relative_validity'] * 86400;
                    $start_time = $couponList['opposite_add_time'];
                }
                if($over_time > $nowTime && $start_time < $nowTime){
     ...
     * @return array|bool
     */
    public function getCouponListById($strCouponId)
    {
        $nowTime = time();
        $param = array(
            'table' => 'Shop\Models\BaiyangCoupon',
            'column' => 'coupon_sn,coupon_name,coupon_description,start_provide_time,end_provide_time,coupon_value,min_cost,coupon_type,bring_number,coupon_number,relative_validity,limit_number,use_range,brand_ids,category_ids,product_ids,wap_url',
            'where' => "WHERE id IN({$strCouponId}) AND (
            ({$nowTime} BETWEEN start_provide_time AND end_provide_time AND relative_validity=0)
            OR relative_validity<>0) AND group_set = 0 AND is_cancel=0 AND wap_platform=1",
            'order' => 'ORDER BY end_provide_time ASC'
        );
        $ret = $this->getData($param);
        return $ret;
