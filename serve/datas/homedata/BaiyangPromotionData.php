...
//        }
        $this->setPromotionCache(); // 设置促销活动缓存
    ...
    public function insertPromotionDetailLog($param) {
        foreach ($param['availPromotionList'] as $value) {
            ...
            if ($value['promotion_type'] != BaiyangPromotionEnum::LIMIT_BUY) {
                ...
                $add_id = $this->addData($addData, true);
                if (!$add_id) {
                    return false;
                }
                // 插入子订单促销日志
                $tempKey = (int)array_search(1, array_column($param['supplierList'], 'supplier_id'));
                $selfOrderSn = $param['supplierList'][$tempKey]['orderSn'];
                if (count($param['supplierList']) > 1) {
                    // 满赠+加价购
                    if (in_array($value['promotion_type'], [BaiyangPromotionEnum::FULL_GIFT, BaiyangPromotionEnum::INCREASE_BUY])) {
                        $addData['bind']['order_sn'] = $selfOrderSn;
                        $addData['bind']['pid'] = $add_id;
                        if (!$this->addData($addData)) {
                            return false;
                        }
                    }
                    // 包邮
                    if ($value['promotion_type'] == BaiyangPromotionEnum::EXPRESS_FREE) {
                        foreach ($param['supplierList'] as $k => $v) {
                            $addData['bind']['order_sn'] = $v['orderSn'];
                            $addData['bind']['pid'] = $add_id;
                            if (!$this->addData($addData)) {
                                return false;
                            }
                    ...
                    // 满减满折
                    if (in_array($value['promotion_type'], [BaiyangPromotionEnum::FULL_MINUS, BaiyangPromotionEnum::FULL_OFF])) {
                        foreach ($param['supplierList'] as $k => $v) {
                            if (isset($value['resultInfo']['supplierList'][$v['supplier_id']])) {
                                $addData['bind']['order_sn'] = $v['orderSn'];
                                $addData['bind']['discount_money'] = $value['resultInfo']['supplierList'][$v['supplier_id']];
                                $addData['bind']['pid'] = $add_id;
                                if (!$this->addData($addData)) {
                                    return false;
                                }
        ...
        if (!empty($param['couponInfo'])) {
            ...
            $coupon_add_id = $this->addData($addData, true);
            if (!$coupon_add_id) {
                return false;
            }
        }
        // 子订单限时优惠和优惠券
        foreach ($param['supplierList'] as $kk => $vv) {
            // 优惠券
            if (count($param['supplierList']) > 1) {
                if ($vv['couponPrice'] > 0) {
                    $addData = array(
            ...
            foreach ($vv['goodsList'] as $k => $v) {
                // 限时优惠
                if (isset($v['discountPromotion']) && !empty($v['discountPromotion']) && $v['discountPromotion']['promotion_type'] == BaiyangPromotionEnum::LIMIT_TIME) {
                    $limitTimeInfo = $v['discountPromotion'];
                    $addData = array(
                        'table' => '\Shop\Models\BaiyangOrderPromotionDetail',
                        'bind'  => array(
                            'order_sn'           => $param['orderSn'],
                    ...
                    $limit_add_id = $this->addData($addData, true);
                    if (!$limit_add_id) {
                        return false;
                    }
                    if (count($param['supplierList']) > 1) {
                        $addData['bind']['order_sn'] = $vv['orderSn'];
                        $addData['bind']['pid'] = $limit_add_id;
                        if (!$this->addData($addData)) {
                            return false;
                        }
                    }
     ...
     * @author 吴俊华
     */
    private function setPromotionCache()
    {
        $promotionCondition = [
            'table' => '\Shop\Models\BaiyangPromotion as pro',
            'join' => 'left join \Shop\Models\BaiyangPromotionRule as rules on pro.promotion_id = rules.promotion_id',
