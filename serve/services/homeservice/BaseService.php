...
use Shop\Home\Datas\BaiyangMerchantGoodsWarehouseRelationData as MGWData;
...
class BaseService extends Component
{
    ...
    public function responseResultJson($status, $explain = '', $data = [], $tipsData = [])
    {
        ...
        $data = empty($data) ? null : $data;
        if (empty($tipsData)) {
            echo json_encode(['error_code' => $status, 'message' => $explain, 'data' => $data]);
            exit;
        }
        $this->regularReplace($explain, $tipsData);
        echo json_encode(['error_code' => $status, 'message' => $explain, 'data' => $data, 'tipsData' => $tipsData]);
        exit;
    }
     ...
     * @author 吴俊华
     */
    public function uniteReturnResult($httpCode, $data = [], $tipsData = [], $line = null)
    {
        if(!empty($httpCode)){
            return $this->responseResult($httpCode, HttpStatus::$HttpStatusMsg[$httpCode], $data, $tipsData, $line);
        }
        return $this->responseResult(HttpStatus::FAILED, HttpStatus::$HttpStatusMsg[HttpStatus::FAILED], $data, $tipsData, $line);
    }
     ...
     * @param array $data 数据
     * @param array $tipsData  提示语里面的变量  eg:[5]  收货地址不能超过5个 (这个5是在后台/程序设置的)
     * @return json array [] 结果信息
     * @author 秦亮
     */
    public function uniteReturnResultJson($httpCode, $data = [], $tipsData = [])
    {
        if(!empty($httpCode)){
            return $this->responseResultJson($httpCode, HttpStatus::$HttpStatusMsg[$httpCode], $data, $tipsData);
        }
        return $this->responseResultJson(HttpStatus::FAILED, HttpStatus::$HttpStatusMsg[HttpStatus::FAILED], $data, $tipsData);
    }

    /**
     * 接口加密sign校验
     * @param filteredPost array $param
     *       -sign string 接口传递加密串 (*)
     *       -appsecret string 秘钥 (*)
     * @return boolean
     * @author 秦亮
     */
    public function checkSign($filteredPost)
    { 
        $checkResult = false;
        if (is_array($filteredPost) && !empty($filteredPost) && isset($filteredPost['appsecret']) && !empty($filteredPost['appsecret']) && isset($filteredPost['sign']) && !empty($filteredPost['sign'])) {
            $ordered_data = $this->order_array($filteredPost);
            if ($ordered_data) {
                $post_string = $this->post_to_string($ordered_data);
                if (!empty($post_string)) {
                    if ($filteredPost['sign'] === strtolower(md5($post_string))) {
                        $checkResult = true;
                    }
                }
            }
    ...
    public function order_array($array)
    {
        ...
        ksort($array); // 根据字母正序排序
        return $array;
    }
    
    /**
     * 将排序后的post数据使用@串联成字符串
     * @param array $post_data 排序后的post数组
     * @param string $key 校验签名所使用的key
     * @return string 串联后的字符串
     */
    public function post_to_string($post_data)
    {
        $string = '';
    ...
    protected function getGoodsPromotionInfo($goodsPromotionList,$basicParam,$action = 'shoppingCart',$recordId = '')
    {
        ...
        $allPromotionInfo = $goodsPromotionList['allPromotionInfo']; //全场促销活动列表
        ...
        $joinGoodsList1 = $this->getJoinGoodsList($joinGoodsPromotion1);
        ...
        if($action != 'shoppingCart'){
            ...
            if(isset($usableCouponsList['couponList']) && !empty($usableCouponsList['couponList'])){
                ...
                //匹配每个商品用到的优惠券优惠金额coupon_total
                $shoppingCartGoodsInfo = $this->_eventsManager->fire('promotion:shoppingCartMatchCouponTotal',$this,['couponGoodsList' => $usableCouponsList['goodsList'],'shoppingCartGoodsInfo' => $shoppingCartGoodsInfo]);
            }
        }
        //优先级在优惠券之后的促销活动
        ...
        if($action != 'shoppingCart'){
            //购物车列表信息匹配使用优惠券后的promotion_total、promotion_price
            if(isset($usableCouponsList['goodsList']) && !empty($usableCouponsList['goodsList'])){
                $shoppingCartGoodsInfo = $this->_eventsManager->fire('promotion:matchCouponPromotionTotal',$this,['shoppingCartGoodsInfo' => $shoppingCartGoodsInfo,'goodsList' => $usableCouponsList['goodsList']]);
            }
        }
        ...
        //计算商品、套餐里每个商品的优惠价
        $shoppingCartGoodsInfo = $this->_eventsManager->fire('promotion:calculateGoodsPromotionPrice',$this,['promotionsInfo' => $shoppingCartGoodsInfo]);
    ...
    private function promotionToListen($promotionGoods, $increaseBuyList = [], &$mutexList, &$joinList)
    {
        ...
        foreach ($promotionGoods as $key => $value) {
            ...
            if (isset($result['supplierList'])) {
                $promotionGoods[$key]['promotion']['resultInfo']['supplierList'] = $result['supplierList'];
            }
	...
	protected function globalOrderPromotionInfo ( array $param)
	{
        ...
        $goodsNumber = isset($param['goods_number']) ? (int)$param['goods_number'] : 0;
        ...
        if ($goodsId > 0 && $goodsNumber > 0) {
            ...
            $goodsId = $goodsNumber = 0;
        }
		// 查询商品库存去除库存不足的商品
		foreach ($list as $k => &$item)
		{
			$stock = $this->func->getCanSaleStock(['goods_id'=> $item['goods_id'], 'platform'=> $platform]);
			$bond  = $this->func->getUsingGoodsWhId($item['goods_id'],$item['goods_number']);
			$shopId = MGWData::getInstance()->getGoodsShopId($item['goods_id']);
			...
			$item['bond'] = $bond;
			$item['shopId'] = $shopId;
			$item['stock_type'] = $goodsInfo['is_use_stock'];
		}
		// 去除因各种原因没选中的商品
		...
		$result['bond'] = $bonds[0];
		$shopIds = array_unique(array_column ($result[ 'goodsList' ], 'shopId'));
		if ( ($action == 'commitOrder') && (count ($shopIds) > 1) )
		{
			return $this->uniteReturnResult (HttpStatus::NOT_SAME_SHOP, ['param' => $param]);
		}
		$result['shopId'] = $shopIds[0];
		if ($action == "orderInfo")
		{
			// 梳理结算页面所需求的字段
        ...
        $result['goodsNumber'] = $goodsNumber;
    ...
    protected function orderPromotionInfo($param) {
        ...
        // 立即购买
        if ($goodsId > 0 && $result['rxExist'] == 1) {
            return $this->uniteReturnResult(HttpStatus::RX_CANNOT_BUY_NOW, ['param'=> $param]);
        }
        $result['goodsId'] = $goodsId;
        $result['goodsNumber'] = $goodsNumber;
        return $this->uniteReturnResult(HttpStatus::SUCCESS, $result);
