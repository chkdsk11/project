...
class GroupfightService extends BaseService
{
    /**
     * @var GroupService
     */
    ...
    public static function getInstance()
    {
        if (empty(static::$instance)) {
            static::$instance = new GroupfightService();
        }
     ...
     *              is_dummy   是否虚拟用户 必填
     *              is_open   是否开团 必填
     *              group_id  活动或者开团id 必填
     *              goods_num  商品数量  必填 写死 1
     *              channel_subid  渠道id 必填
     *              platform    渠道 必填
     *              is_check  是否是检测  选填
     *              address_id  地址id 选填
     * @return \array[]
     */
    public function confirm($param){//return $this->uniteReturnResult(HttpStatus::SUCCESS, []);
        try{
            $data = GroupfightOrderConfirm::getInstance()->confirm($param);

        }catch (Exception $e){
            $tipsData = [];
            if($e->getMessage()){
                $tipsData = @json_decode($e->getMessage(), true);
            }
            return $this->uniteReturnResult($e->getCode(), $param, $tipsData, $e->getLine());
        } catch (\Error $e) {
            return $this->uniteReturnResult($e->getCode(), $param, [], $e->getLine());
        }
    ...
    public function submit($param){
        ...
        try{
            $data = GroupfightOrderSubmit::getInstance()->confirm($param);
        }catch (Exception $e){
            $tipsData = [];
            if($e->getMessage()){
                $tipsData = @json_decode($e->getMessage(), true);
                if($tipsData and isset($tipsData['status'])){
                    $param['error']['status'] = $tipsData['status'];
                    $param['error']['message'] = isset($tipsData['explain']) ? $tipsData['explain'] : '' ;
                    $tipsData = [];
                }
            }
            return $this->uniteReturnResult($e->getCode(), $param, $tipsData, $e->getLine());
    ...
    public function chkGroupFightHasFull($param){
        try{
            $data = [];
            if(empty($param['user_id'])){
                throw new Exception('',HttpStatus::USERID_ERROR);
            }

            if(empty($param['order_id'])){
                throw new Exception('', HttpStatus::PARAM_ERROR);
            }
            $row = BaiyangGroupFightBData::getInstance()->getGroupFightBuyByOrderSn($param['order_id'], $param['user_id']);
            if(empty($row)){
                throw new Exception('', HttpStatus::NO_ORDER);
            }
            $row = $row[0];
            if(isset($row['gf_state']) === false ){
                throw new Exception('', HttpStatus::SYSTEM_ERROR);
            }
            $data['state'] = $row['gf_state'];
        }catch (Exception $e){
            return $this->uniteReturnResult($e->getCode(), $param, [], $e->getLine());
        }
        return $this->uniteReturnResult(HttpStatus::SUCCESS, $data);
...
//        }
    ...
    {
        $data = BaiyangGroupFightData::getInstance()->getGroupFightBuyByOrderSn($orderSn, $userId);
        if (empty($data)) {
            return $this->uniteReturnResult(HttpStatus::NO_DATA);
        }
    ...
    private function getfightUserList($orderSn,$userId)
    {
        ...
        $data = BaiyangGroupFightData::getInstance()->getGroupFightAndUserList($fightGroupAct['gf_id']);
