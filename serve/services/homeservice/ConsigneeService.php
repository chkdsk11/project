...
class ConsigneeService extends BaseService
{
    ...
    public static function getInstance()
    {
        if(empty(static::$instance)){
            static::$instance = new ConsigneeService();
        }
    ...
    public function addOrEditConsignee($param)
    {
        ...
        if (!in_array($action,['add','edit']) || empty($data['user_id']) || empty($data['consignee']) || empty($data['province']) || empty($data['city']) || empty($data['county']) || empty($data['address']) || empty($data['telphone']) || !$this->verifyRequiredParam($param)) {
            return $this->uniteReturnResult(HttpStatus::PARAM_ERROR);
        }
        ...
        $remainNumber = ($configNumber - $consigneeNumber > 0) ? $configNumber - $consigneeNumber : 0;
        // 收货地址信息
        $consigneeList = BaiyangUserConsigneeData::getInstance()->getUserConsigneeList($data['user_id']);
    ...
    public function getRegionList($param)
    {
        ...
        // 存入redis
        $redis = $this->cache;
        $allRegionList = $redis->getValue(CacheKey::REGION_LIST);
        $regionKeysList = $redis->getValue(CacheKey::REGION_KEYS_LIST);
        if(empty($allRegionList) || empty($regionKeysList)){
            ...
            $redis->setValue(CacheKey::REGION_LIST,$allRegionList);
            $redis->setValue(CacheKey::REGION_KEYS_LIST,$regionKeysList);
        }
        if(empty($allRegionList) || empty($regionKeysList)){
