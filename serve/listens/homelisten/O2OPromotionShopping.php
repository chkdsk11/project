...
class O2OPromotionShopping extends BaseListen
{
    /**
                        ...
                        if (!empty($groupInfo)) {
                            ...
                            if ($groupExist) {
                                ...
                                foreach ($groupGoodsList as $goodsInfo) {
                                    ...
                                    $goodsInfo['goods_status'] = $goodsInfo['sale'] == 0 ? 1 : ($goodsInfo['goods_number'] > $goodsInfo['stock'] ? 2 : 1);
     ...
     * @author 柯琼远
     */
    public function getCartGoodsInfo($goods_id) {
        $platform = $this->config->platform;
        $goodsInfo = array();
        $skuInfo = BaiyangSkuData::getInstance()->getSkuInfo($goods_id, $platform);
        ...
        $goodsInfo['rule_name'] = $skuInfo['ruleName'];
    ...
    public function getIncreaseBuyGiftShow($even, $class, $shoppingCartInfo) {
       ...
       /* foreach ($shoppingCartInfo['availPromotionList'] as $key => $value) {
        ...
        foreach ($shoppingCartInfo['goodsList'] as $key => $value) {
            if ($value['group_id'] == 0) {
                $tempGift = [];
                foreach ($value['bind_gift'] as $k => $v) {//print_r($v);
                    $v = $this->giftStockDesc($v, $giftStock);
                    if ($v['goods_number'] > 0) $tempGift[] = $v;
                }
    ...
    public function getshoppingCartField($even, $class, $param) {
        ...
        $result = array(
            'shoppingCart' => [
                ...
                'totalQty' => 0,
                'selectQty' => 0,
                'freeFreightText' => "全场满{$freightInfo[0]}元包邮",
                'freePrice' => "0.00",
                'totalPrice' => "0.00",
            ],
            'globalCart' => [
                ...
                'totalQty' => 0,
                'selectQty' => 0,
                'freeFreightText' => "全场满{$freightInfo[1]}元包邮",
                'freePrice' => "0.00",
                'totalPrice' => "0.00",
            ],
            'isLogin' => 0,
        ...
        foreach ($result['globalCart']['goodsList'] as $key => $value) {
            ...
            if ($value['group_id'] == 0) {
                if ($value['drug_type'] == 1 && $value['selected'] == 1) {
                    $result['globalCart']['rxExist'] = 1;
                }
                ...
                foreach ($value['groupGoodsList'] as $k => $v) {
                    if ($v['drug_type'] == 1 && $value['selected'] == 1) {
                        $result['globalCart']['rxExist'] = 1;
                    }
        ...
        foreach ($result['shoppingCart']['moduleList'] as $key => $value) {
            foreach ($value['goodsList'] as $kk => $vv) {
                ...
                if ($vv['group_id'] == 0) {
                    if ($vv['drug_type'] == 1 && $vv['selected'] == 1) {
                        $result['shoppingCart']['rxExist'] = 1;
                    }
                    ...
                    foreach ($vv['groupGoodsList'] as $k => $v) {
                        if ($v['drug_type'] == 1 && $vv['selected'] == 1) {
                            $result['shoppingCart']['rxExist'] = 1;
                        }
    ...
    public function getOrderInfoField($even, $class, $shoppingCartInfo)
    {
        $result = [
            'goodsList' => $shoppingCartInfo['goodsList'],
            'allIncreaseBuyList' => isset($shoppingCartInfo['allIncreaseBuyList']) ? $shoppingCartInfo['allIncreaseBuyList'] : [],
            'allGiftList' => isset($shoppingCartInfo['allGiftList']) ? $shoppingCartInfo['allGiftList'] : [],
            'couponList' => isset($shoppingCartInfo['couponList']) ? $shoppingCartInfo['couponList'] : [], // 优惠券列表
            'couponInfo' => [], // 选中优惠券信息
            'consigneeList' => array(), // 收货地址列表（先初始化）
            'o2oInfo' => null, // O2O信息
            'paymentList' => array(), // 配送方式信息
            'expressList' => array(), // 配送方式信息
            'freightInfo' => array(), // 运费信息
            'facePayTips' => array(), // 货到付款运费提示信息（PC端）
            'invoiceInfo' => ["if_receipt"=>0], // 发票信息
            'announcement' => "", // 配送公告
            'ifFacePay' => 1, // 是否可选货到付款
            'expressType' => 0, // 配送方式：0-普通快递,1-顾客自提,2-两小时达,3-当日达
            'expressText' => "",
            'goodsIds' => '', // 商品id字符串
            'isDummy' => 1, // 是否虚拟订单
	...
	public function getCommitOrderField($even, $class, $shoppingCartInfo) {
		$result = [
			'goodsList'          => [],
			'availPromotionList' => isset($shoppingCartInfo['availPromotionList']) ? $shoppingCartInfo['availPromotionList'] : [],
			'giftList'           => [],
			'couponInfo'         => [],
			'o2oInfo'            => [],
			'consigneeInfo'      => [],
			...
			'detailDiscountMoney'=> '0.00', // 商品优惠的金额（优惠优惠，疗程，套餐，换购品优惠）
			'freight'            => '0.00',
			'costPrice'          => '0.00', // 应付金额
		];
		foreach ($shoppingCartInfo['goodsList'] as $key => $value) {
			$result['goodsTotalPrice'] = bcadd($result['goodsTotalPrice'], $value['discount_total'], 2);
            $result['detailDiscountMoney'] = bcadd($result['detailDiscountMoney'], bcsub($value['sku_total'], $value['discount_total'], 2), 2);
            if(isset($value['promotionInfo'])){
			...
			unset($value['promotionInfo'],$value['promotion_price'],$value['promotion_total']);
			if ($value['group_id'] == 0) {
				$result['goodsList'][] = $value;
				$result['goodsIds'] .= ','.$value['goods_id'];
				$result['giftList'] = array_merge($result['giftList'], $value['bind_gift']);
            ...
            /*else {
				foreach ($value['groupGoodsList'] as $k => $v) {
					...
					$result['goodsList'][] = $v;
					$result['goodsIds'] .= ','.$v['goods_id'];
					$result['giftList'] = array_merge($result['giftList'], $v['bind_gift']);
    ...
    public function getCommitGlobalOrderField($even, $class, $shoppingCartInfo) {
        $result = [
	        ...
	        'goods_price'       =>  0, //商品总价
	        'insure_amount'     =>  0, //保费
	        'shop_id'           =>  $shoppingCartInfo['shopId'],
        ];
        foreach ($shoppingCartInfo['goodsList'] as $item) {
        ...
        $result['bond'] = implode('', $shoppingCartInfo['bond']);
        return $result;
    }
...
//                }
     ...
     * @return array true|false 结果信息 (true为有参加，false为没参加)
     * @author 吴俊华
     */
    private function isJoinAssignPromotion($salesId,$basicParam)
    {
        ...
        //进行中的限时优惠和限购
        $param['promotion_type'] = BaiyangPromotionEnum::LIMIT_BUY.','.BaiyangPromotionEnum::LIMIT_TIME;
        ...
        foreach ($shoppingCartGoodsInfo as $key => $value){
            if(isset($value['promotionInfo']['default_promotion']) && !empty($value['promotionInfo']['default_promotion'])){
                if($value['promotionInfo']['default_promotion']['promotion_type'] != BaiyangPromotionEnum::LIMIT_BUY){
                    ...
                    $unit = !empty($ruleValue) ? $ruleValue[0]['unit'] : 'yuan';
            ...
            if(!empty($value['promotionInfo']['join_promotionList'])){
                foreach ($value['promotionInfo']['join_promotionList'] as $kk => $vv){
                    if($vv['promotion_type'] != BaiyangPromotionEnum::LIMIT_BUY){
                        ...
                        $unit = !empty($vv['rule_value']) ? $vv['rule_value'][0]['unit'] : 'yuan';
                        $shoppingCartGoodsInfo[$key]['promotionInfo']['join_promotionList'][$kk]['resultInfo'] = [
                            'isCanUse' => false,
                            'bought_number' => 0,
                            'lack_number' => 0,
                ...
                foreach ($value['promotionInfo']['switch_promotion'] as $kk => $vv){
                    foreach ($vv as $k => $v){
                        if($v['promotion_type'] != BaiyangPromotionEnum::LIMIT_BUY){
                            ...
                            $unit = !empty($v['rule_value']) ? $v['rule_value'][0]['unit'] : 'yuan';
     ...
     *   [limitTime]  -array [二维数组]  所有进行中的限时优惠活动 (不是必填的)
     * @return array [] 结果信息
     *       - int     goods_id      商品id
