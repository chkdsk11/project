...
use Shop\Home\Datas\O2OBaiyangCouponData;
use Shop\Models\OrderEnum;

class O2OPromotionCalculate extends BaseListen
{
    /**
    ...
    public function fullMinus($event,$class,$param)
    {
        ...
        foreach($promotion['rule_value'] as $value) {
            ...
            if (bccomp($goodsFull, $value['full_price'], 2) !== -1) {
                ...
                return array('isCanUse'=> $isCanUse, 'reduce_price'=> $reduce_price,'lack_number'=> $lack_number, 'unit'=> $unit, 'goodsList'=> $goodsList,'copywriter' => $copywriter,'message' => $message,'pro_message' => $proMessage, 'bought_number'=> $goodsFull);
            }
    ...
    public function fullGift($event,$class,$param)
    {
        ...
        foreach($promotion['rule_value'] as $ruleValue) {
            ...
            if(bccomp($threshold, $ruleValue['full_price'], 2) !== -1){
                ...
                if(!empty($premiumsGroup)){
                    //判断赠品的库存、上下架状态
                    foreach($premiumsGroup as $kk => $vv){
                        if($premiumsGroup[$kk]['sale'] == 0){
                            unset($premiumsGroup[$kk]);
                            continue;
                        }
                        unset($premiumsGroup[$kk]['sale']);
                    }
    ...
    public function getCouponList($event,$class,$param)
    {
        ...
        $couponData = O2OBaiyangCouponData::getInstance();
        //获取当前用户已经领取的的优惠券
